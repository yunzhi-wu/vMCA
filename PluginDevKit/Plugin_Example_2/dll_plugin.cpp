...
//              It shows how to create a simple plugin that decode rows and alter the row content, a Decoder
...
{
  return (CPlugin_DLL_API_V1*)new CPlugin_Example_2;
}
...
bool Q87_Decoder::pvDecode(char* row_p, unsigned int* length_p, const unsigned int maxLength)
{
  CTextParser parser;
  ...
  // The parser member function search will return true if there is a match with the Time: string. It will also place the parser
  ...
  if (parser.Search(Q87_MATCH, 5))
  {
    if (parser.Search("Value:", 6))
    {
      ...
      sprintf_s(&row_p[startIndex - 1], MAX_STRING_LENGTH - startIndex, "  <DECODED>  %-4.2f", (float)value/(float)128.0);
