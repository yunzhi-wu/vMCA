...
{
  return (CPlugin_DLL_API_V1*)new CPlugin_Example_4;
}
...
void CPlot_Example_4::pvPlotRow(const char* row_p, const unsigned int* length_p, unsigned int rowIndex)
{
  /*
  In the Example_4 we are parsing a text-file that contains information about execution flow. The intention is to plot the execution as a sequence diagram
  with events, messages, and execution bars.
  ...
  exec_msg:       A asynchronous message is sent from one life line to another starting execution, execution of caller is not blocked
  sync_exec_msg:  As above, however this is a synchronous message hence the execution on the src life line is blocked until call returns
  ...
  */
  ...
  if (m_parser.Search("lifeline", 8))
  {
    ...
    sprintf_s(lifeLineName, 128, "LifeLine:%d", lifeLineDest);

    m_lifeLines_a[lifeLineDest] = m_sequenceDiagram_p->AddLifeLine((float)lifeLineDest - 0.25f, (float)lifeLineDest + 0.25f, lifeLineName, (unsigned int)strlen(lifeLineName), RGB(0x55, 20 + 20 * lifeLineDest, 20 + 20 * lifeLineDest));
    return;
  }
  ...
  if (m_parser.Search("op:event", 8))
  {
    ...
    sprintf_s(msgString, 10, "%d", eventID);

    m_sequenceDiagram_p->AddEvent(m_lifeLines_a[lifeLineDest], time, rowIndex, msgString, (unsigned int)strlen(msgString), RGB(90, 90, 90), false);
  ...
  if (m_parser.Search("op:msg", 6))
  {
    ...
    sprintf_s(msgString, 10, "%d", msgID);

    m_sequenceDiagram_p->AddMessage(m_lifeLines_a[lifeLineSrc], time, m_lifeLines_a[lifeLineDest], rowIndex, msgString, (unsigned int)strlen(msgString), RGB(90, 90, 90), false);

    return;
  }


  // 4-5. Search for messages starting execution. Since the syntax for both synchrounous and asynchronous the code is made common. First
  //      check if there is a match against sync_exec_msg or exec_msg.
  ...
  if (exec_op_sync || exec_op)
  {
    ...
    {
    case 1000:
      strcpy_s(msgString, 10, "Start");
      break;

    case 1001:
      strcpy_s(msgString, 10, "Stop");
      break;

    case 1003:
      strcpy_s(msgString, 10, "Reload");
      break;

    default:
      strcpy_s(msgString, 10, "???");
      break;
    }
    ...
    m_parser.Extract(startIndex, endIndex, execName); // extract a text string from the row. This string contains the name of the execution bar
    ...
    if (exec_op_sync)
    {
      m_sequenceDiagram_p->AddMessage(m_lifeLines_a[lifeLineSrc], time, m_lifeLines_a[lifeLineDest], rowIndex, msgString, (unsigned int)strlen(msgString), RGB(90, 90, 90), true, true);
      m_sequenceDiagram_p->AddExecution(m_lifeLines_a[lifeLineDest], time, time + (double)duration, rowIndex, execName, (unsigned int)strlen(execName), RGB(90, 90, 90));
      m_sequenceDiagram_p->AddReturnMessage(m_lifeLines_a[lifeLineDest], time + (double)duration, m_lifeLines_a[lifeLineSrc], rowIndex, "done", 4, RGB(90, 90, 90), true, false);
    }
    else
    {
      m_sequenceDiagram_p->AddMessage(m_lifeLines_a[lifeLineSrc], time, m_lifeLines_a[lifeLineDest], rowIndex, msgString, (unsigned int)strlen(msgString), RGB(90, 90, 90), false, true);
      m_sequenceDiagram_p->AddExecution(m_lifeLines_a[lifeLineDest], time, time + (double)duration, rowIndex, execName, (unsigned int)strlen(execName), RGB(90, 90, 90));
    }
